...
export default class AddName extends Component {
  ...
  render() {
    ...
    let todos = names.map(n => {
      return (
        <Todo
          todoDone={() => this.todoDone(n.id)}
          done={n.done}
          key={n.id}
          name={n.name}
          id={n.id}
          deleteTodo={() => this.deleteTodo(n.id)}
        />
      );
    });
...
let List = ({todos}) => (
  <ScrollView style={styles.scrollContainer}>{todos}</ScrollView>
);
...
let Tabs = ({setFilter, filter}) => (
  <View style={{height: 50, flexDirection: 'row'}}>
    <Tab
      name="SHOW_ALL"
      filter={filter}
      setFilter={() => setFilter('SHOW_ALL')}
      type="All"
    />
    <Tab
      name="SHOW_COMPLETE"
      filter={filter}
      setFilter={() => setFilter('SHOW_COMPLETE')}
      type="Completed"
    />
    <Tab
      name="SHOW_INCOMPLETE"
      filter={filter}
      setFilter={() => setFilter('SHOW_INCOMPLETE')}
      type="Not Completed"
    />
  </View>
);
...
let Tab = ({type, setFilter, filter, name}) => (
  <TouchableHighlight
    ...
    style={[
      ...
      filter == name && {backgroundColor: '#666'},
    ]}
